...
- **Steps to Reproduce:**
  1. Ensure a deactivated visitor exists (the seeds create several, e.g. the "Sam Robinson" record at seed index 18).
...
- **Summary:** Rendering the active list issues one query per visitor for its host (N+1), scaling poorly.
- **Type:** Performance
- **Description:**
...
- **Summary:** Check-in times are generated, stored, and displayed in UTC, never the receptionist's local timezone (Asia/Kathmandu).
- **Type:** Functional (frontend / timezone)
- **Description:**

  The backend saves `checked_in_at` in UTC (Asia/Kathmandu is UTC+5:45), and the frontend then renders that UTC value with `toISOString()` (web/src/VisitorList.jsx:4-6) instead of the receptionist's local timezone.
...
## 6. API errors are silently swallowed; the UI gives no feedback on failures

- **Summary:** Failed requests return `null`, and forms clear as if the operation succeeded.
- **Type:** Usability
- **Description:**
  When a form submission succeeds the input fields clear, and the UI shows no success or error message to indicate whether the action completed or failed.
...
- **Description:**
  ...
  state, so a deactivated visitor who is still checked in remains in the active list (compounds #1). Neither
...
- **Expected Result:** Unsupported page values should either be rejected (400) or normalized deliberately (no silent duplication / suppressed errors).
...
- **1. No authentication / role separation (the README says visitors are deactivated by an administrator).**
...
- **4. Search wildcard matching is unspecified (excluded, not treated as a defect).**
  `Visitors#search` builds `LIKE` patterns from user input, so `%` and `_` act as SQL wildcards
  (`q=%` matches most visitors). The spec does not define search matching rules, so this is recorded
  as an open question rather than a defect; a literal-match fix would not change the API response shape.
