...
and `host_id` as required and shows no error feedback on a failed POST, so a frontend fix
follow-up is needed for the remaining fields (untouched here, as the track is backend-only).
...
2. **Defect 2 (no server-side validation + UTC timestamps)**: It is never good for a backend to
   rely on the frontend sending correct data, so I choose to fix server-side validation. Defect 2 targets
   two server-side gaps that both contradict the spec. (a) The spec requires a full name, company, host,
   and purpose on every registration, but the API happily persisted empty rows; fixing it enforces the
   registration contract. (b) The app is
   ...
   value (`Z` to `+05:45`), so both are deliberately called out above.
...
|--------|-------------------------------|------------------------------------------------|
| API errors silently swallowed (frontend) | Client-side only (`api.js` returns `null` on non-2xx; forms always reset). Needs frontend work; pairs naturally with the Defect 2 frontend follow-up. | No - a fix is purely in the web client. |
| UTC display - frontend half of the timezone defect (report #5) | Server side is fixed as part of Defect 2 (`config.time_zone = "Asia/Kathmandu"`); the remaining bug is client-side: `web/src/VisitorList.jsx` renders with `toISOString()`, which always converts to UTC for display. | No - client-side. |
| `check_out` / `deactivate` state guard (report #7) | Low impact, but a *robust* fix changes the API: rejecting repeat check-outs would introduce `409 Conflict` responses (a new response surface) unless made silently idempotent. Flagged as a contract decision. | **Yes** - a strict fix would add `409` error responses. |
...
| Search treats `%` / `_` as LIKE wildcards | Explicitly excluded rather than fixed: the spec is silent on search matching, so this is treated as unspecified (`defect-report.md`, assumption 4) rather than a defect. | No - same response shape, only match semantics. |
...
Files changed:
- `api/app/controllers/api/visitors_controller.rb` : Defect 1 + Defect 3 fixes.
